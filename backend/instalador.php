...
$dmh_installer_direct_run = realpath($_SERVER["SCRIPT_FILENAME"] ?? "") === __FILE__;

if ($dmh_installer_direct_run) {
...
function dmh_run_instalador(): array {
    ...
    try {
        ...
        $camposOpiniones = [
            "estado" => "ALTER TABLE opiniones ADD COLUMN estado ENUM('pendiente','aprobada','rechazada') NOT NULL DEFAULT 'aprobada' AFTER comentario",
        ...
        foreach ($camposOpiniones as $nombreColumna => $sqlAlter) {
            ...
            if ((int)$stmtColOpiniones->fetchColumn() === 0) {
                $conexion->exec($sqlAlter);
                $pasos[] = "Columna opiniones.$nombreColumna añadida";
            }
        }
        ...
        $conexion->exec("
            UPDATE opiniones
            SET estado = CASE
                WHEN estado IN ('aprobada', 'rechazada', 'pendiente') THEN estado
                ELSE 'aprobada'
            END
        ");
        ...
        // Creamos el administrador por defecto solo si no existe.
        // Si el usuario ya fue editado manualmente, no sobrescribimos ni su rol
        // ni su contraseña al volver a ejecutar el instalador.
